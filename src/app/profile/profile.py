...
# Get Profile
@router.get('/profile')
async def get_profile(userUID=Depends(verify_access_token), db: Session = Depends(db.get_default_db)):
    # Get User
    user = crud.get_user_uid(db=db, uid=userUID)
...
async def update_profile(fname:  Annotated[str, Form()], lname:  Annotated[str, Form()],
                         dname: Annotated[str, Form()], userUID=Depends(verify_access_token), db: Session = Depends(db.get_default_db)):
...
async def update_password(currentpassword: Annotated[str, Form()], newpassword: Annotated[str, Form()], userUID=Depends(verify_access_token), db: Session = Depends(db.get_default_db)):
